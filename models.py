...
class FiLMBlock(nn.Module):
    """
    FiLM modulation between MLP layers for per-point networks (zero-init -> identity start).
    """
    ...
    def forward(self, h: torch.Tensor, emb: torch.Tensor) -> torch.Tensor:
        """
        h: (B*N, C)
        emb: (B, E); should be broadcast externally to (B*N, E)
        """
        h = self.norm(h)
...
class VelocityNet(nn.Module):
    """
    Per-point MLP: v_theta(x, t, cond) in R^{point_dim}
    - point_dim=3: geometry
    - point_dim=6: geometry + color
    """
...
class ShapeEncoder(nn.Module):
    ...
    Supports in_channels=3 (xyz) or 6 (xyz+rgb).
    """
...
class GRL(nn.Module):
    ...
    def forward(self, x, lambd: float | None = None):
        return _GRLFn.apply(x, self.lambd if lambd is None else float(lambd))

# --- Optional: adversarial head (with dropout) ---
class CondAdversary(nn.Module):
    """Predict cond (joint) from z. Used with GRL to remove cond info in z."""
...
class _FiLM1d(nn.Module):
    """
    (B, C, N) FiLM: Norm -> (1 + gamma) * x + beta; gamma/beta are zero-initialized (identity start).
    """
...
class _PVBlock(nn.Module):
    """
    Single PVConv block: PVConv -> SharedMLP (1x1) -> FiLM -> residual.
    - normalize=True stabilizes voxel neighborhoods (recommended).
    """
...
class _PVStage(nn.Module):
    """
    Stage: 1x1 SharedMLP channel projection -> k * PVBlock.
    """
...
class ContextNet(nn.Module):
    """
    Multi-scale PVConv pyramid producing per-point context features (ctx).
    + t-gated context blending: use global (t, cond) context for small t, and PVConv multi-scale context for large t.
    """
    def __init__(self,
                 in_point_dim: int,              # 3 / 6
                 cond_dim: int,                  # z (+ joint)
                 emb_dim: int = 256,             # t/cond embedding dimension
                 ctx_dim: int = 64,              # output context dimension
        ...
        # Stem input channels: [emb | xyz | (rgb)]
        ...
        # Output head: multi-scale aggregation -> ctx_dim
        ...
        # ---- Global context generated only from (t, cond) (used with t-gating) ----
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, cond: Optional[torch.Tensor]) -> torch.Tensor:
        ...
        # Do heavy context computation in FP32 for stability
        with torch.amp.autocast("cuda", enabled=False):
            ...
            ctx_pv = self.head_out(h).permute(0, 2, 1)  # (B,N,ctx_dim)  --- PVConv multi-scale context

            if self.use_t_gate:
                # Build a global context from emb and blend it via t-gating
...
class VelocityNetWithContext(nn.Module):
    """
    Per-point MLP head mapping [x_i | ctx_i | emb(t, cond)] -> v_i.
    Initialization / CFG / FiLM behavior matches the base VelocityNet.
    """
...
class HybridMLP(nn.Module):
    """
    Point-flow backbone:
      - ContextNet (PVConv pyramid) produces per-point context features (ctx).
      - VelocityNetWithContext regresses per-point velocity.
      - forward(x, t, cond, cond_drop_mask=None) -> v
      - guided_velocity(x, t, cond, guidance_scale=0.0)
      - set_bn_eval(freeze=True): optionally freeze BatchNorm stats during training.
    """
    def __init__(self,
                 cond_dim: int,
                 point_dim: int = 3,
                 ...
                 with_global: bool = True,
                 voxel_normalize: bool = True,
                 # t-gating (tunable; conservative defaults)
                 use_t_gate: bool = True,
                 t_gate_k: float = 10.0,
        ...
        self.ctx_net = ContextNet(
            ...
            # Pass through t-gating parameters
    ...
    def set_bn_eval(self, freeze: bool = True):
        """
        Freeze / unfreeze BatchNorm layers.
        - If SharedMLP/PVConv contains BN/SyncBN, they will be set to eval() when frozen.
        - GroupNorm is unaffected.
        """
