...
def save_point_cloud_ply_rgb(xyz: torch.Tensor, rgb: torch.Tensor, path: str):
    ...
    Args:
        xyz: (N, 3) float coordinates (torch.Tensor or numpy array).
        rgb: (N, 3) colors; either float in [0, 1] or uint8 in [0, 255]
    ...
    if rgb_t.dtype.is_floating_point:
        # Convert float RGB in [0, 1] to uint8 in [0, 255] (with rounding).
